...
> **✅ 전 단계 완료(2026-09-14).** 배포 + **실기기 설치·검증까지 끝**. 안드로이드 폰에 WebAPK로 설치되어 정상 동작 확인(로그인·하단바·복사·오프라인 4종 전부 통과, 수정 필요 없었음).
> 이 문서는 재작업/스토어 전환 시 기준으로 남겨둔다. 상세는 CHANGELOG v0.17.
...
- [x] **폰 설치 완료** — 안드로이드 크롬에서 WebAPK 설치 확인
- [ ] 안드로이드 크롬에서 라이브 접속 → **"앱 설치"** 배너/메뉴 확인
- [ ] 설치 → 앱 서랍에 아이콘 / 주소창 없음 / 최근앱 독립 카드 확인
- [ ] 스플래시 확인
- [ ] 비행기모드 → 실행되는지(오프라인) 확인

### ✅ Stage 6 — 실기기 검증 (완료 · **4종 전부 통과, 대응 불필요**)
- [x] **구글 로그인 — ✅ 정상 동작**. standalone에서 `signInWithPopup`이 그대로 작동.
  - ➡️ **`signInWithRedirect` 교체는 불필요**. §10 저장 게이트를 건드릴 이유가 없어졌다. ⚠️ 나중에 "PWA니까 redirect로 바꾸자"는 생각이 들면 **하지 말 것** — 실기기에서 popup이 통과했다(2026-09-14).
- [x] **세이프에어리어 — ✅ 문제 없음**. 하단바(합계+복사 CTA)가 제스처 바에 **안 물린다**(실기기 확인).
  - ➡️ **`env(safe-area-inset-bottom)` 적용 불필요 — CSS 변경 0.** ⚠️ 없는 문제를 고치지 말 것. 나중에 기기/OS가 바뀌어 실제로 물리는 걸 **눈으로 본 뒤에만** 대상 3곳(`.total-bar`·`.overlay-panel`·`.toast`)에 적용.
- [x] 바텀시트 · 메뉴 설정 풀페이지 · 뒤로가기 동작 확인
- [ ] 복사(`navigator.clipboard`) 동작 확인 — 이 앱의 최종 목적
- [x] **오프라인 실행** — ✅ 비행기모드에서 정상 실행
- [x] **업데이트 반영** — 배포된 실제 SW로 검증 완료(캐시에 옛 버전을 심어도 최신 반환 + 캐시 자동 복구). 네트워크 우선 전략이 의도대로 동작.

### ✅ Stage 7 — 문서 (완료)
- [x] `CHANGELOG.md` v0.17 항목
- [x] `TODO.md` "PWA 설치 셸" 체크
- [x] `HANDOFF.md` 최종 갱신 줄 + §8 + §10 결정 4건 추가
