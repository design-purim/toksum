...
- **클라우드 저장은 "로드 성공 후에만"(저장 게이트)** (2026-08-02, 데이터 소실 사고 대응) — 로그인 시 **빈/오래된 로컬 폴더가 Firestore를 덮어써** 실사용자 메뉴가 전량 사라진 사고가 있었음. 원인: `setUser`의 notify가 클라우드 로드 *전에* 로컬 저장을 예약 → 로드 실패/지연 시 빈 값이 클라우드를 덮음. **수정: cloud.js에 `enableFolderSync/disableFolderSync/queueFolderSave` 게이트.** 인증 콜백 진입 즉시 저장 잠금 → `syncOnLogin`이 반영/첫업로드에 **성공한 뒤에만** 해제, **실패 세션은 잠금 유지**. main.js는 `queueFolderSave`만 호출(디바운스·게이트는 cloud.js 소유). ⚠️ **이 게이트를 우회해 로그인 직후 무조건 저장하는 코드로 되돌리지 말 것** — 소실 재발. `saveUserFolders`는 여전히 `setDoc`(통째 덮어쓰기)이라 게이트가 유일한 방어선. 상세는 CHANGELOG(2026-08-02 fix)·§11.
